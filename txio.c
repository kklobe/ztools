    ...
    for (i = 0; i < 3; i++) {
      for (j = 0; j < 26; j++) {
        if ((unsigned int)header.alphabet) {
          lookup_table[i][j] =
              (char)get_byte((unsigned int)header.alphabet + (i * 26) + j);
        } else {
  ...
  do {
    ...
    for (i = 10; i >= 0; i -= 5) {
      ...
      if (synonym_flag) {
        ...
        addr = (unsigned long)get_word((unsigned int)header.abbreviations +
                                       synonym + (code * 2)) *
               2;
        ...
        if (code == 0) {
          ...
          /*
           * The use of the synonym and shift codes is the only difference
          ...
          if ((unsigned int)header.version < V3) {
            ...
            if (code == 1) {
               ...
               *
               * Shift keys 2 & 3 only shift the next character and can be used
               * regardless of the state of the shift lock. Shift keys 4 & 5
               * lock the shift until reset.
               *
               * The following code implements the the shift code state
               * transitions:
               *
               *               +-------------+-------------+-------------+-------------+
               ...
               * | Code        |      2      |       3     |      4      | 5 |
               * +-------------+-------------+-------------+-------------+-------------+
               * | lowercase   | uppercase   | punctuation | uppercase   |
               * punctuation | | uppercase   | punctuation | lowercase   |
               * punctuation | lowercase   | | punctuation | lowercase   |

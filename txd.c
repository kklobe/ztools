  ...
  if (decode.first_pass) {
    if (decode.pc < decode.initial_pc) {
      ...
      if (flag && (unsigned int)header.version < V5) {
        ...
        vars = (char)read_data_byte(&pc);
        low_pc = decode.pc;
        for (pc = pc + (vars * 2) - 1, flag = 0; pc >= low_pc && flag == 0;
             pc -= story_scaler) {
          decode.pc = pc;
          for (i = 0, flag = 1; i < 3 && flag; i++) {
            pcindex = 0;
...
static int decode_opcode(void) {
  ...
  case VARIABLE_OPERAND:
    ...
    switch (code) {
      ...
      caseline(0x35, "SOUND_EFFECT", NUMBER, NUMBER, NUMBER, ROUTINE, NONE,
               PLAIN);
    ...
    default:
      ...
      case V3:
        switch (code) {
          caseline(0x20, "CALL", ROUTINE, ANYTHING, ANYTHING, ANYTHING, STORE,
                   CALL);
      ...
      case V4:
        switch (code) {
          ...
          caseline(0x20, "CALL_VS", ROUTINE, ANYTHING, ANYTHING, ANYTHING,
                   STORE, CALL);

          caseline(0x24, "READ", LOW_ADDR, LOW_ADDR, NUMBER, ROUTINE, NONE,
                   PLAIN);

          caseline(0x29, "PULL", VAR, NIL, NIL, NIL, NONE, PLAIN);

          caseline(0x2C, "CALL_VS2", ROUTINE, ANYTHING, ANYTHING, ANYTHING,
                   STORE, CALL);
          ...
          caseline(0x36, "READ_CHAR", NUMBER, NUMBER, ROUTINE, NIL, STORE,
                   PLAIN);
          caseline(0x37, "SCAN_TABLE", ANYTHING, LOW_ADDR, NUMBER, NUMBER, BOTH,
                   PLAIN);
        }
      case V5:
      case V7:
      case V8:
        switch (code) {
          ...
          caseline(0x24, "READ", LOW_ADDR, LOW_ADDR, NUMBER, ROUTINE, STORE,
                   PLAIN);
          ...
          caseline(0x39, "CALL_VN", ROUTINE, ANYTHING, ANYTHING, ANYTHING, NONE,
                   CALL);
          caseline(0x3A, "CALL_VN2", ROUTINE, ANYTHING, ANYTHING, ANYTHING,
                   NONE, CALL);
          caseline(0x3B, "TOKENISE", LOW_ADDR, LOW_ADDR, LOW_ADDR, NUMBER, NONE,
                   PLAIN);
          caseline(0x3C, "ENCODE_TEXT", LOW_ADDR, NUMBER, NUMBER, LOW_ADDR,
                   NONE, PLAIN);
          caseline(0x3D, "COPY_TABLE", LOW_ADDR, LOW_ADDR, NUMBER, NIL, NONE,
                   PLAIN);
          caseline(0x3E, "PRINT_TABLE", LOW_ADDR, NUMBER, NUMBER, NUMBER, NONE,
                   PLAIN);
          caseline(0x3F, "CHECK_ARG_COUNT", NUMBER, NIL, NIL, NIL, BRANCH,
                   PLAIN);
        }
      case V6:
        switch (code) {
          ...
          caseline(0x2D, "ERASE_WINDOW", NUMBER, NIL, NIL, NIL, NONE, PLAIN);
          caseline(0x2E, "ERASE_LINE", NUMBER, NIL, NIL, NIL, NONE, PLAIN);
          caseline(0x2F, "SET_CURSOR", NUMBER, NUMBER, NUMBER, NIL, NONE,
                   PLAIN);
          ...
          caseline(0x3C, "ENCODE_TEXT", LOW_ADDR, NUMBER, NUMBER, LOW_ADDR,
                   NONE, PLAIN);
          caseline(0x3D, "COPY_TABLE", LOW_ADDR, NUMBER, LOW_ADDR, NIL, NONE,
                   PLAIN);
          caseline(0x3E, "PRINT_TABLE", LOW_ADDR, NUMBER, NUMBER, NUMBER, NONE,
                   PLAIN);
          caseline(0x3F, "CHECK_ARG_COUNT", NUMBER, NIL, NIL, NIL, BRANCH,
  ...
  case PATTR:
    if (decode.first_pass == 0) {
      if ((int)value == 1)
        ...
        tx_printf("%s",
                  (option_inform) ? "scripting_enable" : "SCRIPTING_ENABLE");
      else if ((int)value == 3)
        tx_printf("%s",
                  (option_inform) ? "redirect_enable" : "REDIRECT_ENABLE");
      else if ((int)value == 4)
        ...
        tx_printf("%s",
                  (option_inform) ? "scripting_disable" : "SCRIPTING_DISABLE");
      else if ((int)value == -3)
        tx_printf("%s",
                  (option_inform) ? "redirect_disable" : "REDIRECT_DISABLE");
      else if ((int)value == -4)
  ...
  if (parser_type >= inform_gv2) {
    ...
    for (i = 0; i < parse_count; i++) {
      ...
      if (routine == addr) {
        ...
        show_syntax_of_parsing_routine(i, verb_table_base, verb_count,
                                       parser_type, prep_type, prep_table_base,
                                       attr_names_base);
      }
    }

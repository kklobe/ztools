...
void show_header(void) {
  ...
  for (i = 0; i < 8; i++) {
    if ((unsigned int)header.config & (1 << i)) {
      tx_printf("%s%s", (list++) ? ", " : "",
                ((unsigned int)header.version < V4) ? interpreter_flags1[i]
  ...
  tx_printf("Serial number:            %c%c%c%c%c%c\n", (char)header.serial[0],
            (char)header.serial[1], (char)header.serial[2],
            (char)header.serial[3], (char)header.serial[4],
            (char)header.serial[5]);

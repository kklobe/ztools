  ...
  *prep_type = 0;
  ...
  if ((*parser_type < inform5_grammar) && (unsigned int)header.version == V6) {
    ...
    *verb_data_base = area2base;
    /* there is no preposition table, but *prep_table_base bounds the verb data
     * area */
...
void show_verbs(int symbolic) {
  ...
  configure_parse_tables(&verb_count, &action_count, &parse_count, &parser_type,
...
static void show_verb_parse_table(unsigned long verb_table_base,
  ...
  for (i = 0; (unsigned int)i < verb_count; i++) {
    ...
    if (parser_type == infocom6_grammar) {
      ...
      verb_address =
          (unsigned int)address; /* cast is guaranteed to work provided unsigned
                                    int >= 16 bits */
      ...
      if (verb_entry != 0xFFFF)
        show_verb_grammar(parse_entry, verb_address, (int)parser_type, 0, 0, 0L,
                          0L);
      ...
      if (do_address) {
        ...
        while (entry_count--) {
          ...
          tx_printf("] ");
          show_verb_grammar(parse_entry, verb_address, (int)parser_type, 1, 0,
                            0L, 0L);
          tx_printf("\n");
        }
      }
      if (doio_address) {
        ...
        while (entry_count--) {
          ...
          tx_printf("] ");
          show_verb_grammar(parse_entry, verb_address, (int)parser_type, 2, 0,
                            0L, 0L);
          tx_printf("\n");
        }
      }
      ...
      while (entry_count--) {
        ...
        if (parser_type < inform_gv2) {
          ...
          /* GV2 variable entry format
             <flags and action high> <action low> n*(<token type> <token data 1>
             <token data 2>) <ENDIT>*/
        ...
        show_verb_grammar(parse_entry, VERB_NUM(i, parser_type),
                          (int)parser_type, 0, (int)prep_type, prep_table_base,
                          attr_names_base);
        tx_printf("\n");
      }
    }
    ...
    if (parser_type == infocom6_grammar) {
      ...
      if (doio_address) {
        ...
        while (entry_count--) {
          ...
          verb_entry += 8; /* skip preposition and direct object and preposition
                              and indirect object*/
        }
      }
    } else {
...
int is_gv2_parsing_routine(unsigned long parsing_routine,
  ...
  unsigned long parsing_routine_packed =
      (parsing_routine - (unsigned long)story_scaler * header.routines_offset) /
      code_scaler;
    ...
    while (entry_count--) {
      ...
      if (parser_type < inform_gv2) {
        ...
        while (object_count) {
          ...
          if (val < 0xb0) {
            ...
            if (val >= 0x10 && val < 0x70 &&
                ((val - 0x10) & 0x1f) == (unsigned int)parsing_routine)
              found = 1;
          }
        }
        ...
        while (val != ENDIT) {
          token_data = read_data_word(&verb_entry);
          if (((val & 0xC0) == 0x80) &&
              (token_data == parsing_routine_packed)) /* V7/V6 issue here */
            found = 1;
          val = (unsigned int)read_data_byte(&verb_entry);
        }
      }
      if (found) {
        show_verb_grammar(parse_entry, VERB_NUM(i, parser_type),
                          (int)parser_type, (int)prep_type, 0, prep_table_base,
                          attr_names_base);
        tx_printf("\n");
      }
    }
...
show_action_tables(unsigned long verb_table_base, unsigned int verb_count,
  ...
  if ((parser_type >= inform5_grammar) && (parser_type < inform_gv2)) {
    ...
    for (action = 0; (unsigned int)action < parse_count; action++) {
      ...
      show_syntax_of_parsing_routine((unsigned long)action, verb_table_base,
                                     verb_count, parser_type, prep_type,
                                     prep_table_base, attr_names_base);
      tx_fix_margin(0);
    }
  }
...
void show_verb_grammar(unsigned long verb_entry, unsigned int verb_index,
  ...
  if (parser_type == infocom6_grammar) {
    ...
    while (val != ENDIT) {
      if (((val & 0x30) == 0x10) ||
          ((val & 0x30) == 0x30)) /* 2nd ... nth byte of alternative list */
        tx_printf(" /");
      tx_printf(" ");
      token_type = val & 0xF;
      token_data = read_data_word(&address);
